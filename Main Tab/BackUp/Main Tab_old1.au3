...
Func MainTab_Handler(ByRef $this, Const ByRef $event)
	...
	Switch $event.ID
		...
		Case "Canvas Tab Selected"
			_GUICtrlTab_ActivateTab($this.Canvas, $this.Tab)
			
					Return True
					
		Case "Parameters Tab Selected"
			_GUICtrlTab_ActivateTab($this.Parameters, $this.Tab)
			
					Return 
					
		Case "Script Tab Selected"
			_GUICtrlTab_ActivateTab($this.Script, $this.Tab)
			
					Return True
					
		Case "Object Explorer Tab Selected"
			_GUICtrlTab_ActivateTab($this.ObjectExplorer, $this.Tab)
			
					Return True
					
		Case "Settings Tab Selected"
			_GUICtrlTab_ActivateTab($this.Settings, $this.Tab)
		...
		Case $GUI_EVENT_PRIMARYUP
			Switch _GUICtrlTab_HitTest($this.Tab, (MouseGetPos(0) - $this.Left), (MouseGetPos(1) - $this.Top))[0]
				Case $this.Canvas
					$messageQueue.Push($messageQueue.CreateEvent("Main Tab", "Canvas Tab Selected"))
					
					Return True
					
				Case $this.Parameters
					$messageQueue.Push($messageQueue.CreateEvent("Main Tab", "Parameters Tab Selected"))
					
					Return True

				Case $this.Script
					$messageQueue.Push($messageQueue.CreateEvent("Main Tab", "Script Tab Selected"))
					
					Return True

				Case $this.ObjectExplorer
					$messageQueue.Push($messageQueue.CreateEvent("Main Tab", "Object Explorer Tab Selected"))
					
					Return True

				Case $this.Settings
					$messageQueue.Push($messageQueue.CreateEvent("Main Tab", "Settings Tab Selected"))
