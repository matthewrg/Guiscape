...
Func Guiscape()
	...
	_AutoItObject_AddMethod($this, "GUIEvent". True)
	_AutoItObject_AddMethod($this, "Announce". True)
	_AutoItObject_AddMethod($this, "Subscribe". True)
	
	Return $this
	
	Local $mainForm = MainForm()
	Local $menubar = Menubar()
	Local $toolbar = Toolbar()
	Local $maintab = Maintab()
	Local $canvas = Canvas()
	Local $parameters = Parameters()
;~ 	Local $script = Script()
;~ 	Local $objectExplorer = ObjectExplorer()
;~ 	Local $settings = Settings()
;~ 	Local $guiObjects = GUIObjects()
...
;~ 	$messageQueue.Subscribe($guiObjects)	
	...
	GUIRegisterMsg($WM_NCLBUTTONDOWN, NCClickedEvent)
