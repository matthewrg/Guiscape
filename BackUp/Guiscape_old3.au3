...
Func Guiscape_GUIObjectsHandler(ByRef $this, Const ByRef $event)
	Switch $event.ID
		Case "Create Form"
			Local Const $form = $this.GUIObjects.CreateForm()
		...
		Case $NC_CLICKED
			Print(WinGetTitle($this.GUIObjects.GetForm($event.Form)) & ": Title Bar")
			
			_WinAPI_SetWindowSubclass($this.GUIObjects.GetForm($event.Form), $pWndProc, 1000)

			$this.ParametersHandler(CreateMessage("Initialize Form", $this.GUIObjects.GetForm($event.Form)))
