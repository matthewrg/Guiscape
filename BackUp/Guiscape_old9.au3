		...
		Switch $event.ID
			...
			Case Else
				Switch $Guiscape.State
					Case $Guiscape.Default
						$Guiscape.Handler($event)

					Case $Guiscape.CreateButton
						$Guiscape.CreateButtonHandler($event)
				EndSwitch
		EndSwitch
	Until False
EndFunc   ;==>main
...
Func Guiscape()
	...
	$this.AddMethod("CreateButtonHandler", "Guiscape_CreateButtonHandler")
	...
	$this.AddProperty("State", $ELSCOPE_READONLY, 1)
	$this.AddProperty("Default", $ELSCOPE_READONLY, 1)
	$this.AddProperty("CreateButton", $ELSCOPE_READONLY, 2)
...
Func Guiscape_Handler(ByRef $this, Const ByRef $event)
	...
	Switch $this.FormManager.Handler($event)
		Case True
			Local Const $form = $this.FormManager.GetForm($event.Form)
	...
	Switch $this.Toolbar.Handler($event.ID)
		...
		Case "Button"
			$this.State = $this.CreateButton

			$this.FormManager.ActiveForm.State = $this.CreateButton
...
Func Guiscape_CreateButtonHandler(ByRef $this, Const $event)
	Switch $this.Canvas.Handler($event)
		Case "Canvas"
			$this.FormManager.SetActiveForm(Null)
	...
	Switch $this.FormManager.ActiveForm.Handler($event)
		Case True
			$this.State = $this.Default
